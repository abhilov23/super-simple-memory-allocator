...
//heap memory works in LIFO form, which means 
//we need to figure out the size, the free location and the poiner to the next memory location 
...
void *malloc(size_t size) //the size_t variable is unsigned integer use to store the size, it only stores 0 and +ve value
{
	size_t total_size;
	void *block;
	header_t *header;
    ...
    //sbrk(0) returns the current address of the block
    //sbrk(x) with positive values increases the brk by x
    //sbrk(-x) with negative values decreases the brk by x, but it cannot decrease the size of memory allocator function because of size_t variable
